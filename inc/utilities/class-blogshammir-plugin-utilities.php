 ...
 * @since 1.0.0
 */
class Blogshammir_Plugin_Utilities {
	...
	/**
	 * Main Blogshammir Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return Blogshammir_Plugin_Utilities
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blogshammir_Plugin_Utilities ) ) {
	...
	public function activate_plugin( $plugin ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error(
				...
				esc_html__( 'Current user can\'t activate plugins', 'blogshammir' )
			);
		}
		...
		if ( empty( $plugin ) ) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__( 'Missing plugin data.', 'blogshammir' )
			);
		}
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Plugin %s is not installed.', 'blogshammir' ),
					$plugin
				)
			);
		}
		...
		do_action( 'blogshammir_plugin_activated_' . $plugin );
	}
	...
	public function deactivate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__( 'Plugin %s is not active.', 'blogshammir' ),
					$plugin
				)
			);
		}
		...
		do_action( 'blogshammir_plugin_deactivated_' . $plugin );
	}
	...
	public function get_recommended_plugins() {
		...
		return apply_filters( 'blogshammir_recommended_plugins', $plugins );
	}
 ...
 *
 * Example: <?php $blogshammir_plugin_utilities = blogshammir_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function blogshammir_plugin_utilities() {
	return Blogshammir_Plugin_Utilities::instance();
}
