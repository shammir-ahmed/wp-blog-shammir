;( function( $ ) {
 	...
 	wp.customize.controlConstructor['blogshammir-range'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.find( '.blogshammir-range-wrapper' ).rangeControl({
		...
		save: function() {
			...
			if ( false === devices || devices === undefined || 0 == devices.length ) {
				value.value = parseFloat( this.container.find( '.blogshammir-range-input' ).val() );
			} else {
				for ( var device in devices ) {
					value[ device ] = parseFloat( this.container.find( '.control-responsive.' + device + ' .blogshammir-range-input' ).val() );
				}
			}

			if ( this.container.find( '.blogshammir-control-unit' ).length ) {
				value.unit = this.container.find( '.blogshammir-control-unit input:checked' ).val();
			}
	...
	( function( $, undef ) {
		...
		RangeControl = {
			...
			_create: function() {
				...
				self.unit = el.find( '.blogshammir-control-wrap' ).attr( 'data-unit' );
			...
			_addListeners: function() {
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'keyup input paste', '.blogshammir-range-input', function() {
				...
				self.element.on( 'click', 'input[type=range]', function() {
					...
					range = $( this );
					input = range.parent().children( '.blogshammir-range-input' );
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'click', '.blogshammir-reset-range', function() {

					var $el;

					self.element.find( '.blogshammir-reset-range' ).each( function( index, el ) {

						$el = $( el );

						// Reset range unit.
						self.unit = $el.data( 'reset_unit' );
						$el.closest( '.blogshammir-range-wrapper' ).find( '.blogshammir-control-unit input[type="radio"][value="' + $el.data( 'reset_unit' ) + '"]' ).click();
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'click', '.blogshammir-control-unit input', function() {

					var $unit = $( this );

					self.element.find( '.blogshammir-control-wrap input[type="range"]' ).each( function( index, el ) {
					...
					if ( 'px' === $unit.val() && ( 'rem' === self.unit || 'em' === self.unit ) ) {
						...
						self.element.find( 'input.blogshammir-range-input' ).each( function( index, el ) {
			...
			_initResponsiveControls: function() {
				...
				var self = this,
					el = self.element,
					wrap = el.find( '.blogshammir-control-wrap' );

				el.addClass( 'blogshammir-control-responsive' );

				// Populate responsive switcher.
				if ( el.find( '.customize-control-title' ).length ) {

					var $switcher = $( '<ul class="blogshammir-responsive-switchers"></ul>' );

					for ( var device in self.options.responsive ) {
						$switcher.append( '<li class="' + device + '"><span class="preview-' + device + '" data-device="' + device + '"><span class="blogshammir-tooltip small-tooltip">' + device + '</span><i class="' + self.options.responsive[device].icon + '"></i></span></li>' );
					}
			...
			_initUnit: function() {
				...
				var template = wp.template( 'blogshammir-control-unit' );
				...
				if ( 'object' === typeof self.options.unit ) {
					...
					$( '<span class="blogshammir-range-unit">' + self.options.unit + '</span>' ).insertAfter( wrap.find( '.blogshammir-range-input' ) );
				}

				wrap.parent().find( '.blogshammir-control-unit input:checked' ).click();
			}
		};

		// Register the color picker as a widget.
		$.widget( 'blogshammir.rangeControl', RangeControl );
	}( jQuery ) );
