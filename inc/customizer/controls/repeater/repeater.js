...
*/
function BlogshammirLinkField($el) {
    ...
    const open = () => {
        ...
        var $textarea = jQuery('<textarea id="blogshammir-link-textarea" style="display:none;"></textarea>');
        if(!jQuery('#blogshammir-link-textarea').length)jQuery('body').append($textarea);

        var val = getNodeValue();
        wpLink.open('blogshammir-link-textarea', val.url, val.title, null);
    }
    ...
    const  onClose = () => {
        ...
        jQuery('#blogshammir-link-textarea').remove();
      }
...
( function( api, $ ) {
    api.controlConstructor['blogshammir-repeater'] = api.Control.extend({
        ready: function() {
            var control = this;
            setTimeout( function() {
                control._init();
        ...
        _init: function() {
            ...
            control.autocorrect_range_input_number = function( input_number, timeout ) {
                ...
                var range_input = input_number,
                    ...
                    reset = parseFloat( range.find( '.blogshammir-reset-range' ).attr( 'data-reset_value' ) ),
            ...
            if ( ! window._upload_blogshammir ) {
                ...
                window._upload_blogshammir = wp.media({
            ...
            window._upload_blogshammir.on( 'close', function() {

                // get selections and save to hidden input plus other AJAX stuff etc.
                var selection = window._upload_blogshammir.state().get( 'selection' );

            
            });
            window.media_current = {};
            window.media_btn = {};
            window._upload_blogshammir.on( 'select', function() {

                // Grab our attachment selection and construct a JSON representation of the model.
                var media_attachment = window._upload_blogshammir.state().get( 'selection' ).first().toJSON();
            ...
            control.handleMedia = function( $context ) {
                $( '.item-media', $context ).each( function() {
                    ...
                    $( '.upload-button, .attachment-media-view', _item ).on( 'click', function( e ) {
                        ...
                        window._upload_blogshammir.open();
                    });
                });
            };
            ...
            control.handleGradient = function( $context ) {
                var control = this,
                    ...
                    range_input,
                    value,
                    blogshammir_range_input_number_timeout,
                ...
                $( '.item-gradient', $context ).each( function() {
                    ...
                    _item.find( '.blogshammir-range-wrapper' ).each( function() {
                        var $this = $( this );
                        ...
                        $this.find( 'input.blogshammir-range-input' ).on( 'change keyup', function() {
                            control.autocorrect_range_input_number( $( this ), 1000, blogshammir_range_input_number_timeout );
                        }).on( 'focusout', function() {
                            control.autocorrect_range_input_number( $( this ), 0, blogshammir_range_input_number_timeout );
                        });
                    ...
                    _item.on( 'change', '.blogshammir-select-wrapper', function() {
                        control.update_value();
                    });
            ...
            control.handleBackground = function( $context ) {
                ...
                $( '.item-design-options', $context ).each( function() {
                    ...
                    // Linked button
                    _item.on( 'click', '.blogshammir-spacing-linked', function() {
                    ...
                    // Unlinked button
                    _item.on( 'click', '.blogshammir-spacing-unlinked', function() {
					...
					$.each(
						...
						function ( fieldId, field ) {
							if ( field.type === 'select' && field.is_select2 ) {
								...
								if ( $select.length ) {
									var select2Config = {
										placeholder: field.placeholder || ( typeof blogshammir_customizer_localized !== 'undefined' ? blogshammir_customizer_localized.strings.select_category : 'Select' ),
										allowClear: true,
									...
									if ( field.data_source ) {
										select2Config.ajax = {
											...
											error: function(error) {
												console.error( typeof blogshammir_customizer_localized !== 'undefined' ? blogshammir_customizer_localized.strings.error_loading_data : 'Error loading data', error );
											}
										};
									}
            ...
            control.linkPicker = function( $context ){
                if(item_link = $context.find('.blogshammir-field-link')) {
                  const a =  new BlogshammirLinkField(item_link);
                  a.init();
                }
            }
            ...
            control.actions = function( $context ) {
                if ( control.params.live_title_id ) {
                    ...
                    if ( 'blogshammir_section_order_styling' === control.id ) {
...
jQuery( document ).ready( function( $ ) {
    ...
    var options_font_type = '',
        icon_group = '';
    $.each( Blogshammir_Icon_Picker.fonts, function( key, font ) {
        font = $.extend({}, {
            url: '',
            name: '',
            prefix: '',
    ...
    icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Blogshammir_Icon_Picker.search );
