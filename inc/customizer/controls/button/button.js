;(function( $ ) {
	...
	wp.customize.controlConstructor['blogshammir-button'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.button', function(e){
				...
				if ( action ) {
					...
					var data = {
						_ajax_nonce: blogshammir_customizer_localized.wpnonce,
						action: action,
					};

			 		$.post( blogshammir_customizer_localized.ajaxurl, data, function(response) {
