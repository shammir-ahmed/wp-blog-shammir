;(function($) {
	...
	wp.customize.controlConstructor['blogshammir-widget'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.blogshammir-widget-container .widget-top', function(){
				$(this).closest( '.widget' ).toggleClass( 'blogshammir-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});
			
			// Minimize widget content when clicked on Done
			control.container.on( 'click', '.blogshammir-widget-container .widget-control-close', function() {
				$(this).closest( '.widget' ).toggleClass( 'blogshammir-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});

			// Show available widgets
			control.container.on( 'click', '.blogshammir-add-widget', function(e) {
				e.preventDefault();
				control.updateList();
			});
		 	...
		 	control.container.on( 'click', '.blogshammir-widget-edit-nav', function(){
		 		wp.customize.control( 'nav_menu_locations[' + $(this).closest( '.blogshammir-widget-nav-container' ).data( 'menu-location' ) + ']' ).focus();
		 		control.close();
		 	});
			...
			$( control.container ).find( '.blogshammir-widget-nav-container' ).each( function(){
			...
			wp.customize( 'nav_menu_locations[' + menu_location + ']', function( value ) {
				value.bind( function( newval ) {
					...
					if ( newval ) {
						...
						$container.addClass( 'blogshammir-widget-nav-has-menu' )
							.find( '.blogshammir-widget-nav-name' )
							.html( menu_name );
					} else {
						$container.removeClass('blogshammir-widget-nav-has-menu');
					}
		...
		updateList: function(){
			...
			if ( widgets ) {
				...
				// Hide all widgets.
				$( '#blogshammir-available-widgets-list .blogshammir-widget' ).hide().removeClass('disabled');

				// Display allowed widgets.
				$.each( widgets, function( index, el ) {

					widget = $( '#blogshammir-available-widgets-list #blogshammir-widget-tpl-blogshammir_customizer_widget_' + index );

					widget.show();

					if ( el.hasOwnProperty( 'max_uses' ) && el.max_uses > 0 && el.max_uses <= $(self.container).find('.blogshammir-widget-container [data-widget-type="' + index + '"]').length ) {
						widget.addClass('disabled');
					}
				});
			} else {
				// Show all widgets
				$( '#blogshammir-available-widgets-list .blogshammir-widget' ).show();
			}
		},
		...
		addWidget: function( widget_id_base ) {
			...
			// Get widget form
			widget_html = $.trim( $(this.container).find( '.blogshammir-widget-tpl-' + widget_id_base ).html() );
			widget_html = widget_html.replace( /<[^<>]+>/g, function( m ) {
				return m.replace( /__i__|%i%/g, widget_uuid );
			} );

			// Append new widget.
			var $widget = $( widget_html ).appendTo( this.container.find( '.blogshammir-widget-container' ) );
			...
			if ( $widget.find( '.blogshammir-widget-nav-container' ).length ) {
				this.bindMenuLocation( $widget.find( '.blogshammir-widget-nav-container' ) );
			}
		},

		close: function() {
			$( 'body' ).removeClass( 'blogshammir-adding-widget' );
		},

		update: function() {

			// Get all widgets in the area
			var widgets = this.container.find( '.blogshammir-widget-container .widget' );
		...
		setupSortable: function() {
			...
			$( this.container ).find( '.blogshammir-widget-container' ).sortable({
				items: '> .widget',
				handle: '.widget-top',
				intersect: 'pointer',
				axis: 'y',
 	...
 	$(document).ready( function(){
	 	...
	 	$( '.wp-full-overlay' ).on( 'click', '.blogshammir-add-widget, .blogshammir-close-widgets-panel', function(e) {
	 		e.preventDefault();

	 		$( 'body' ).toggleClass( 'blogshammir-adding-widget' );

	 		if ( $( this ).data( 'location-title' ) ) {
	 			control = wp.customize.control( $(this).data('control') );
	 			$( '#blogshammir-available-widgets' ).attr( 'data-control', control.params.id ).find( '.blogshammir-widget-caption' ).find( 'h3' ).html( $(this).data( 'location-title' ) );
	 		}
	 	});

	 	$( '.wp-full-overlay' ).on( 'click', '.customize-section-back', function(e) {
	 		$( 'body' ).removeClass( 'blogshammir-adding-widget' );
	 		$( '#blogshammir-available-widgets' ).removeAttr( 'data-control' );
	 	});

		// Add widget to widget control.
	 	$( '#blogshammir-available-widgets' ).on( 'click', '.blogshammir-widget', function(e) {

	 		// Get active control.
			control = wp.customize.control( $( '#blogshammir-available-widgets' ).attr('data-control') );
