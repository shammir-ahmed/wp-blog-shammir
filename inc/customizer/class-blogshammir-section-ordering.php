...
/**
 * Class Blogshammir_Section_Ordering
 */
class Blogshammir_Section_Ordering {
	...
	/**
	 * Main Blogshammir_Section_Ordering Instance.
	 *
	 * @since 1.0.0
	 * @return Blogshammir_Section_Ordering
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blogshammir_Section_Ordering ) ) {
	...
	public function __construct() {
		...
		add_filter( 'blogshammir_section_priority', array( $this, 'get_section_priority' ), 10, 2 );
	}
	...
	public function get_section_priority( $value, $key = '' ) {
		...
		$orders = get_theme_mod( 'blogshammir_sections_order' );
		if ( empty( $orders ) ) {
			return $value;
		}
	...
	public function refresh_positions() {
		$section_order         = get_theme_mod( 'blogshammir_sections_order' );
		$section_order_decoded = json_decode( $section_order, true );
		if ( ! empty( $section_order_decoded ) ) {
			remove_all_actions( 'blogshammir_sections' );
	...
	private function hook_section_by_slug( $slug, $priority ) {
		...
		add_action( 'blogshammir_sections', $slug, absint( $priority ) );
	}
}


function blogshammir_section_reordering() {
	return Blogshammir_Section_Ordering::instance();
}
blogshammir_section_reordering();
