; ( function( api, $ ) {
	...
	// Extends our custom "blogshammir-info" section. Make it visible.
	api.sectionConstructor['blogshammir-info'] = api.Section.extend({
	...
	api.blogshammirCustomizerPreviewer = {
		init: function() {
			var self = this,
				control;
	...
	// Store old previewer.
	var blogshammirOldPreviewer = api.Previewer;
	api.Previewer = blogshammirOldPreviewer.extend({
		initialize: function( params, options ) {

			// Store a reference to the Previewer
			api.blogshammirCustomizerPreviewer.preview = this;

			// Call the old Previewer's initialize function
			blogshammirOldPreviewer.prototype.initialize.call( this, params, options );
		}
	});

	api( 'blogshammir_info_style', function( setting ) {
		...
		setting.bind( function( value ) {
			if ( 12 == value ) {
				...
				api.control( 'blogshammir_info_section_spacing' ).setting( data );

				// console.log( 'New value', api.control( 'blogshammir_info_section_spacing' ).setting() );
			} else {
				var data = {
			...
			var selector = api.control( 'blogshammir_info_section_spacing' ).selector;
	...
	_.each( blogshammir_customizer_localized.preview_url_for_section, function( url, id ) { // eslint-disable-line camelcase
		if ( url ) {
	...
	$( document ).ready( function( $ ) {
		...
		api.blogshammirCustomizerPreviewer.init();

		// Display the first responsive control
		$( '.blogshammir-control-responsive' ).each( function() {
			$( this ).find( '.control-responsive' ).first().addClass( 'active' );
		});

		// Responsive switchers
		$( '.customize-control' ).on( 'click', '.blogshammir-responsive-switchers span', function( event ) {

			var $this = $( this ),
				$switcherContainer = $this.closest( '.blogshammir-responsive-switchers' ),
				$switcherButtons = $switcherContainer.find( 'li span' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.blogshammir-control-responsive' ),
		...
		$( '.wp-full-overlay-footer .devices button' ).on( 'click', function( event ) {
			...
			// Set up variables
			var $this = $( this ),
				$devices = $( '.customize-control .blogshammir-responsive-switchers' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.blogshammir-control-responsive' );
        ...
        setTimeout(() => {
             // Tooltip positioning
            if ( $( '.blogshammir-tooltip' ).length ) {
                
                var $tooltip, $iconPosLeft, $iconPosRight, $titleWidth;

                $( '.blogshammir-tooltip' ).each( function() {
                    $tooltip = $( this );
                    ...
                    $titleWidth = $tooltip.closest( '.blogshammir-control-wrapper' ).outerWidth();

                    $iconPosLeft = $tooltip.closest( '.blogshammir-info-icon' ).css( 'position', 'static' ).position().left;
		...
		api.hasOwnProperty( 'previewer' ) && ( api.previewer.bind( 'blogshammir-customize-disable-section', function( e ) {
			$( 'input[id=' + e + ']' ).trigger( 'click' ).trigger( 'change' );
			const val = ! api.control( e ).setting.get();
			api.control( e ).setting.set( val );
		}), api.previewer.bind( 'blogshammir-customize-focus-control', function( e ) {
			api.control( e ).focus();
		}) );

		//Scroll to section
		$( 'body' ).on( 'click', '#sub-accordion-panel-blogshammir_panel_homepage > .accordion-section.control-subsection', function( event ) {
