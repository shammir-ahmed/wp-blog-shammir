//--------------------------------------------------------------------//
// Blogshammir script that handles our admin functionality.
//--------------------------------------------------------------------//
...
;( function( $ ) {
	...
	var BlogshammirAdmin = {
		...
		init: function() {
			...
			// Document ready
			$( document ).ready( BlogshammirAdmin.ready );

			// Window load
			$( window ).on( 'load', BlogshammirAdmin.load );

			// Bind UI actions
			BlogshammirAdmin.bindUIActions();

			// Trigger event when Blogshammir fully loaded
			$( document ).trigger( 'blogshammirReady' );
		},
		...
		*/
		bindUIActions: function() {
			...
			$wrap.on( 'click', '.plugins .hester-btn:not(.active)', function( e ) {
				...
				BlogshammirAdmin.pluginAction( $this );
			});

			$( document ).on( 'wp-plugin-install-success', BlogshammirAdmin.pluginInstallSuccess );
			$( document ).on( 'wp-plugin-install-error',   BlogshammirAdmin.pluginInstallError );
		},
		...
		pluginInstallSuccess: function( event, response ) {
			...
			BlogshammirAdmin.pluginAction( $button );
		},
	...
	}; // END var BlogshammirAdmin

	BlogshammirAdmin.init();
	window.blogshammiradmin = BlogshammirAdmin;
