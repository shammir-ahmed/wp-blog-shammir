...
if ( ! class_exists( 'Blogshammir_Admin' ) ) :

	/**
	 * Admin Class
	 */
	class Blogshammir_Admin {
		...
		public function __construct() {
			 ...
			 * After admin loaded
			 */
			do_action( 'blogshammir_admin_loaded' );
		}
		...
		private function includes() {
			 ...
			 */
			require_once BLOGSHAMMIR_THEME_PATH . '/inc/admin/helpers.php'; // phpcs:ignore

			/**
			 * Include Blogshammir welcome page.
			 */
			require_once BLOGSHAMMIR_THEME_PATH . '/inc/admin/class-blogshammir-dashboard.php'; // phpcs:ignore

			/**
			 * Guten block
			 */
			// require_once BLOGSHAMMIR_THEME_PATH . '/inc/admin/guten-block.php'; // phpcs:ignore

			/**
			 * Include Blogshammir meta boxes.
			 */
			require_once BLOGSHAMMIR_THEME_PATH . '/inc/admin/metabox/class-blogshammir-meta-boxes.php'; // phpcs:ignore
		...
		public function load_assets( $hook ) {
			 ...
			 */
			if ( ! blogshammir_is_admin_page( $hook ) ) {
				return;
			}
			 ...
			 */
			wp_enqueue_style(
				'blogshammir-admin-styles',
				BLOGSHAMMIR_THEME_URI . '/inc/admin/assets/css/blogshammir-admin' . $suffix . '.css',
				false,
			 ...
			 */
			wp_enqueue_script(
				'blogshammir-admin-script',
				BLOGSHAMMIR_THEME_URI . '/inc/admin/assets/js/' . $prefix . 'blogshammir-admin' . $suffix . '.js',
			 ...
			 */
			$texts = array(
				'install'               => esc_html__( 'Install', 'blogshammir' ),
				'install-inprogress'    => esc_html__( 'Installing...', 'blogshammir' ),
				'activate-inprogress'   => esc_html__( 'Activating...', 'blogshammir' ),
				'deactivate-inprogress' => esc_html__( 'Deactivating...', 'blogshammir' ),
				'active'                => esc_html__( 'Active', 'blogshammir' ),
				'retry'                 => esc_html__( 'Retry', 'blogshammir' ),
				'please_wait'           => esc_html__( 'Please Wait...', 'blogshammir' ),
				'importing'             => esc_html__( 'Importing... Please Wait...', 'blogshammir' ),
				'currently_processing'  => esc_html__( 'Currently processing: ', 'blogshammir' ),
				'import'                => esc_html__( 'Import', 'blogshammir' ),
				'import_demo'           => esc_html__( 'Import Demo', 'blogshammir' ),
				'importing_notice'      => esc_html__( 'The demo importer is still working. Closing this window may result in failed import.', 'blogshammir' ),
				'import_complete'       => esc_html__( 'Import Complete!', 'blogshammir' ),
				'import_complete_desc'  => esc_html__( 'The demo has been imported.', 'blogshammir' ) . ' <a href="' . esc_url( get_home_url() ) . '">' . esc_html__( 'Visit site.', 'blogshammir' ) . '</a>',
			);

			$strings = array(
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'wpnonce'       => wp_create_nonce( 'blogshammir_nonce' ),
				'texts'         => $texts,
				'color_pallete' => array( '#F43676', '#06cca6', '#2c2e3a', '#e4e7ec', '#f0b849', '#ffffff', '#000000' ),
			);

			$strings = apply_filters( 'blogshammir_admin_strings', $strings );

			wp_localize_script( 'blogshammir-admin-script', 'hester_strings', $strings );
		}
		...
		public function admin_footer() {
			...
			if ( ! blogshammir_is_admin_page( $base ) || blogshammir_is_admin_page( $base, 'hester_wizard' ) ) {
				return;
			}
			?>
		...
		public function admin_notices() {
			...
			// Display on Dashboard, Themes and Blogshammir admin pages.
			if ( ! in_array( $screen->base, array( 'dashboard', 'themes' ), true ) && ! blogshammir_is_admin_page() ) {
				return;
			}

			// Display if not dismissed and not on Blogshammir plugins page.
			if ( ! blogshammir_is_notice_dismissed( 'blogshammir_notice_recommended-plugins' ) && ! blogshammir_is_admin_page( false, 'blogshammir-plugins' ) ) {

				$plugins = blogshammir_plugin_utilities()->get_recommended_plugins();
				$plugins = blogshammir_plugin_utilities()->get_deactivated_plugins( $plugins );
				...
				if ( is_array( $plugins ) && ! empty( $plugins ) ) {
					...
					$message = sprintf( wp_kses( __( 'BlogShammir theme recommends the following plugins: %1$s.', 'blogshammir' ), blogshammir_get_allowed_html_tags() ), $plugin_list );

					$navigation_items = blogshammir_dashboard()->get_navigation_items();

					blogshammir_print_notice(
						array(
							'type'        => 'info',
							'message'     => $message,
							...
							'action_text' => esc_html__( 'Install Now', 'blogshammir' ),
						)
					);
				}
			}
