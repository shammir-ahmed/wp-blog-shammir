 ...
 */
function blogshammir_nav_menu_item_args( $args, $item, $depth ) {

	if ( blogshammir_option( 'main_nav_sub_indicators' ) ) {
		$dropdown_indicator = blogshammir()->icons->get_svg( 'chevron-down' );
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			if ( false === strpos( $args->link_after, $dropdown_indicator ) ) {
				$args->link_after .= $dropdown_indicator;
				$args->after       = '<button type="button" class="blogshammir-mobile-toggen">' . $dropdown_indicator . '</button>';
			}
		} else {
...
add_filter( 'nav_menu_item_args', 'blogshammir_nav_menu_item_args', 10, 3 );
 ...
 */
function blogshammir_nav_menu_social_icons( $item_output, $item, $depth, $args ) {

	// Get supported social icons.
	$social_icons = blogshammir_social_links_icons();

	// Change SVG icon inside social links menu if there is supported URL.
	if ( false !== strpos( $args->menu_class, 'blogshammir-socials-menu' ) ) {

		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace(
					$args->link_after,
					'</span><span class="' . esc_attr( $value ) . '" title="' . esc_attr( ucfirst( $value ) ) . '">' . blogshammir()->icons->get_svg( $value, array( 'aria-hidden' => 'true' ) ) . blogshammir()->icons->get_svg(
						$value,
						array(
							'class'       => 'bottom-icon',
							'aria-hidden' => 'true',
...
add_filter( 'walker_nav_menu_start_el', 'blogshammir_nav_menu_social_icons', 10, 4 );
 ...
 */
function blogshammir_social_links_icons() {
	...
	/**
	 * Filter Blogshammir social links icons.
	 *
	 * @since 1.0.0
	 * @param array $social_links_icons Array of social links icons.
	 */
	return apply_filters( 'blogshammir_social_links_icons', $social_links_icons );
}

if ( ! class_exists( 'Blogshammir_Icons' ) ) :
	/**
	 * Blogshammir Icons class.
	 *
	 * @since 1.0.0
	 */
	class Blogshammir_Icons {
		/**
		...
		public function get_svg( $icon = '', $args = array() ) {
			...
			// .blogshammir-icon is a required class.
			if ( false === strpos( $args['class'], 'blogshammir-icon' ) ) {
				$args['class'] = trim( 'blogshammir-icon ' . $args['class'] );
			}
		 ...
		 */
		public function get() {
			return apply_filters( 'blogshammir_icons_svg', self::$icons );
		}
		...
		public function get_meta_icon( $slug = '', $icon = '', $post_id = '' ) {
			...
			if ( is_single( $post_id ) ) {
				if ( blogshammir_option( 'single_entry_meta_icons' ) ) {
					$return = $icon;
				}
			} elseif ( ! is_single() ) {
				if ( blogshammir_option( 'entry_meta_icons' ) ) {
					$return = $icon;
				}
			}

			return apply_filters( 'blogshammir_' . $slug . '_meta_icon', $return, $post_id );
		}
