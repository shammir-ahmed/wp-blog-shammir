<?php
/**
 * Blogshammir compatibility class for Elementor.
 *
 * @package BlogShammir
...
if ( ! class_exists( 'Blogshammir_Elementor' ) ) :

	/**
	 * Elementor compatibility.
	 */
	class Blogshammir_Elementor {
		 ...
		 *
		 * @since 1.0.0
		 * @return Blogshammir_Elementor
		 */
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blogshammir_Elementor ) ) {
				self::$instance = new Blogshammir_Elementor();
			}
		...
		public function enqueue_editor_style() {
			...
			wp_enqueue_style(
				'blogshammir-elementor-editor',
				BLOGSHAMMIR_THEME_URI . '/assets/css/compatibility/elementor-editor-style' . $blogshammir_suffix . '.css',
				false,
		...
		public function setup_postdata() {
			...
			if ( ! blogshammir_enable_page_builder_compatibility() ) {
				return;
			}
			...
			$id = blogshammir_get_the_id();

			$setup = get_post_meta( $id, '_blogshammir_page_builder_setup', true );

			if ( isset( $post ) && empty( $setup ) && ( is_admin() || is_singular() ) && empty( $post->post_content ) && $this->is_built_with_elementor( $id ) ) {

				update_post_meta( $id, '_blogshammir_page_builder_setup', true );
				update_post_meta( $id, 'blogshammir_disable_page_title', true );
				update_post_meta( $id, 'blogshammir_disable_breadcrumbs', true );
				update_post_meta( $id, 'blogshammir_disable_thumbnail', true );
				update_post_meta( $id, 'blogshammir_sidebar_position', 'no-sidebar' );

				update_post_meta( $id, '_wp_page_template', 'page-templates/template-blogshammir-fullwidth.php' );
			}
		...
		public function enqueue_additional() {
			...
			wp_enqueue_style(
				'blogshammir-elementor',
				BLOGSHAMMIR_THEME_URI . '/assets/css/compatibility/elementor' . $blogshammir_suffix . '.css',
				false,
				BLOGSHAMMIR_THEME_VERSION,
				'all'
			);
...
/**
 * Returns the one Blogshammir_Elementor instance.
 */
function blogshammir_elementor() {
	return Blogshammir_Elementor::instance();
}

blogshammir_elementor();
