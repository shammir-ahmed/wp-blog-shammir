...
if ( ! blogshammir_is_woocommerce_activated() ) {
	add_action( 'activate_woocommerce/woocommerce.php', array( blogshammir_dynamic_styles(), 'delete_dynamic_file' ) );
	return;
}

/**
 * Blogshammir WooCommerce Compatibility.
 */
if ( ! class_exists( 'Blogshammir_Woocommerce' ) ) :
	...
	class Blogshammir_Woocommerce {
		 ...
		 *
		 * @since 1.0.0
		 * @return Blogshammir_Woocommerce
		 */
		public static function instance() {

			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blogshammir_Woocommerce ) ) {
				self::$instance = new Blogshammir_Woocommerce();
		...
		private function includes() {
			...
			require BLOGSHAMMIR_THEME_PATH . '/inc/compatibility/woocommerce/woocommerce-functions.php'; // phpcs:ignore
			require BLOGSHAMMIR_THEME_PATH . '/inc/compatibility/woocommerce/class-blogshammir-customizer-woocommerce.php'; // phpcs:ignore
		}
		...
		private function actions() {
			...
			if ( ! is_admin() ) {
				...
				// Disable Blogshammir page description.
				add_filter( 'blogshammir_page_header_description', array( $this, 'shop_remove_page_description' ) );
				...
				// Extend Blogshammir breadcrumb trail.
				add_filter( 'blogshammir_breadcrumb_trail_items', array( $this, 'breadcrumbs' ), 20, 2 );
				add_filter( 'blogshammir_post_type_archive_title', array( $this, 'breadcrumb_post_type_archive_title' ), 10, 2 );

				// Remove WooCommerce sidebar.
				remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar' );
				add_action( 'blogshammir_woocommerce_sidebar', 'woocommerce_get_sidebar' );
				...
				// Replace WooCommerce pagination with Blogshammir pagination.
				remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
				add_action( 'woocommerce_after_shop_loop', 'blogshammir_pagination' );

				// Add back to shop button to Empty Cart.
				add_action( 'woocommerce_cart_is_empty', 'blogshammir_wc_empty_cart_button' );
				...
				add_filter( 'woocommerce_sale_flash', 'blogshammir_wc_add_percentage_to_sale_badge', 20, 3 );

				// Out of stock product badge.
				add_action( 'woocommerce_before_shop_loop_item_title', 'blogshammir_wc_out_of_stock_badge', 10 );
				add_action( 'woocommerce_before_single_product_summary', 'blogshammir_wc_out_of_stock_badge', 10 );
				...
				add_action( 'woocommerce_widget_shopping_cart_buttons', 'blogshammir_wc_widget_shopping_cart_buttons', 10 );
				...
				add_filter( 'woocommerce_subcategory_count_html', 'blogshammir_wc_cat_count_filter', 10, 2 );
				add_filter( 'woocommerce_rating_filter_count', 'blogshammir_wc_rating_count_filter', 10, 3 );
				add_filter( 'woocommerce_layered_nav_count', 'blogshammir_wc_layered_count_filter', 10, 3 );
			...
			// Enqueue styles.
			add_action( 'blogshammir_enqueue_scripts', array( $this, 'enqueue' ) );
			...
			// Add correct sidebar.
			add_filter( 'blogshammir_sidebar_name', array( $this, 'set_sidebar' ) );

			// Set sidebar position.
			add_filter( 'blogshammir_default_sidebar_position', array( $this, 'set_default_sidebar_position' ) );

			// Remove item from cart.
			add_action( 'wp_ajax_blogshammir_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );
			add_action( 'wp_ajax_nopriv_blogshammir_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );

			// Add theme supports.
			add_action( 'after_setup_theme', array( $this, 'theme_supports' ), 20 );

			// Add customizer cart widget.
			add_filter( 'blogshammir_customizer_widgets', array( $this, 'add_customizer_cart_widget' ) );
			add_filter( 'blogshammir_main_header_widgets', array( $this, 'add_cart_to_main_header_widgets' ) );
			...
			// Add dynamic CSS.
			add_filter( 'blogshammir_dynamic_styles', array( $this, 'dynamic_css' ), 5 );

			// Update dynamic styles on deactivation.
			add_action( 'deactivate_woocommerce/woocommerce.php', array( blogshammir_dynamic_styles(), 'delete_dynamic_file' ) );

			// Return Shop page ID.
			add_filter( 'blogshammir_get_the_id', array( $this, 'get_the_id' ) );
		...
		public function theme_supports() {
			...
			if ( blogshammir_option( 'wc_product_gallery_zoom' ) ) {
				add_theme_support( 'wc-product-gallery-zoom' );
			}

			// Product Gallery Lightbox.
			if ( blogshammir_option( 'wc_product_gallery_lightbox' ) ) {
		...
		public function enqueue() {
			...
			wp_enqueue_style(
				'blogshammir-woocommerce',
				BLOGSHAMMIR_THEME_URI . '/assets/css/compatibility/woocommerce' . $blogshammir_suffix . '.css',
				false,
			...
			wp_enqueue_script(
				'blogshammir-wc',
				BLOGSHAMMIR_THEME_URI . '/assets/js/' . $blogshammir_dir . 'blogshammir-wc' . $blogshammir_suffix . '.js',
				array( 'jquery' ),
				BLOGSHAMMIR_THEME_VERSION,
				true
			);
		...
		public function product_catalog_elements() {
			...
			$elements = blogshammir_option( 'product_catalog_elements' );
		...
		public function template_loop_category() {
			...
			if ( is_array( $product_cats ) && ! empty( $product_cats ) ) {
				foreach ( $product_cats as $product_cat ) {
					$cats[] = '<a class="blogshammir-loop-product__category" href="' . esc_url( get_term_link( $product_cat, 'product_cat' ) ) . '">' . esc_html( $product_cat->name ) . '</a>';
				}
			}

			echo '<span class="blogshammir-loop-product__category-wrap">' . wp_kses_post( implode( ', ', $cats ) ) . '</span>';
		}
		...
		public function result_wrapper_start() {
			...
			echo '<div class="blogshammir-woo-before-shop clearfix">';
		}
		...
		public function cart_widget_count_fragment( $fragments ) {
			...
			$fragments['.blogshammir-header-widget__cart a.blogshammir-cart'] = blogshammir_wc_cart_icon( false );
		...
		public function cart_widget_dropdown_fragment( $fragments ) {
			...
			$fragments['.blogshammir-header-widget__cart .dropdown-item'] = blogshammir_wc_cart_dropdown( false );
		...
		public function content_wrapper_end() {
			?>
				...
				<?php do_action( 'blogshammir_woocommerce_sidebar' ); ?>
		...
		public function single_product_wrapper_start() {
			echo '<div class="blogshammir-wc-product-wrap">';
		}
		...
		public function single_product_wrapper_end() {
			echo '</div><!-- END .blogshammir-wc-product-wrap -->';
		}
		...
		public function product_actions() {
			...
			// Remove Blogshammir page title on WooCommerce product pages.
			add_filter( 'blogshammir_page_header_has_title', '__return_false' );

			// Disable Comments toggle.
			add_filter( 'blogshammir_display_comments_toggle', '__return_false' );
		}
		...
		public function add_customizer_cart_widget( $widgets ) {
			...
			$widgets['cart'] = 'Blogshammir_Customizer_Widget_Cart';
		...
		public function register_wc_sidebars() {
			...
			register_sidebar(
				apply_filters(
					'blogshammir_woocommerce_sidebar_name',
					array(
						'name'          => esc_html__( 'WooCommerce Sidebar', 'blogshammir' ),
						'id'            => 'blogshammir-wc-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce pages except Product pages.', 'blogshammir' ),
						'before_widget' => '<div id="%1$s" class="blogshammir-sidebar-widget blogshammir-widget widget %2$s">',
						'after_widget'  => '</div>',
			...
			register_sidebar(
				apply_filters(
					'blogshammir_woocommerce_product_sidebar_name',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'blogshammir' ),
						'id'            => 'blogshammir-wc-product-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce Product pages.', 'blogshammir' ),
						'before_widget' => '<div id="%1$s" class="blogshammir-sidebar-widget blogshammir-widget widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h4 class="widget-title">',
						'after_title'   => '</h4>',
		...
		public function set_sidebar( $sidebar_name ) {
			...
			if ( is_product() ) {
				$sidebar_name = 'blogshammir-wc-product-sidebar';
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$sidebar_name = 'blogshammir-wc-sidebar';
			}
		...
		public function set_default_sidebar_position( $position ) {
			...
			if ( is_product() ) {
				$position = blogshammir_option( 'wc_product_sidebar_position' );
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$position = blogshammir_option( 'wc_sidebar_position' );
			}

			if ( is_product() || is_woocommerce() || is_cart() || is_checkout() ) {
				if ( 'default' === $position ) {
					return blogshammir_option( 'sidebar_position' );
				} else {
					return $position;
				}
			}
		...
		/**
		 * Remove Blogshammir page description on WooCommerce pages.
		 *
		 * @since 1.0.0
		...
		public function remove_item_from_cart() {
			...
			check_ajax_referer( 'blogshammir-nonce' );
		...
		public function product_image_swap() {
			...
			$hover_style = blogshammir_option( 'shop_product_hover' );
			...
			if ( 'image-swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo wp_kses_post( apply_filters( 'blogshammir_woocommerce_product_image_swap', wp_get_attachment_image( $attachment_id, $image_size, false, array( 'class' => 'show-on-hover' ) ) ) );
				}
			}
		}
		...
		public function loop_product_thumb_wrap_start() {
			...
			$class = 'blogshammir-product-thumb';

			if ( 'image-swap' === blogshammir_option( 'shop_product_hover' ) ) {
		...
		public function loop_product_thumb_wrap_end() {
			echo '</div><!-- END .blogshammir-product-thumb -->';
		}
		...
		public function load_customizer_widget() {
			...
			$path = BLOGSHAMMIR_THEME_PATH . '/inc/compatibility/woocommerce/class-blogshammir-customizer-widget-cart.php';
		...
		public function review_order_heading() {
			...
			echo wp_kses_post( apply_filters( 'blogshammir_review_order_heading', '<h3>' . __( 'Payment', 'blogshammir' ) . '</h3>' ) );
		}
		...
		public function checkout_shipping_heading() {
			...
			echo wp_kses_post( apply_filters( 'blogshammir_checkout_shipping_heading', '<h3>' . __( 'Shipping', 'blogshammir' ) . '</h3>' ) );
		}
		...
		public function single_product_related_products_args( $args ) {
			...
			$columns = intval( blogshammir_option( 'wc_related_columns' ) );
			$rows    = intval( blogshammir_option( 'wc_related_rows' ) );
		...
		public function woocommerce_cross_sell_display() {
			...
			if ( blogshammir_option( 'wc_cross_sell_products' ) ) {

				$rows = intval( blogshammir_option( 'wc_cross_sell_rows' ) );
		...
		public function woocommerce_upsell_display() {
			...
			if ( blogshammir_option( 'wc_upsell_products' ) ) {

				$columns = intval( blogshammir_option( 'wc_upsells_columns' ) );
				$rows    = intval( blogshammir_option( 'wc_upsells_rows' ) );
		...
		public function woocommerce_related_products() {
			...
			if ( blogshammir_option( 'wc_related_products' ) ) {
		...
		public function single_product_slider_options( $options ) {
			...
			if ( ! blogshammir_option( 'wc_product_slider_arrows' ) ) {
				return $options;
			}

			$options['directionNav'] = true;
			$options['prevText']     = blogshammir_animated_arrow( 'left', false );
			$options['nextText']     = blogshammir_animated_arrow( 'right', false );
		...
		public function related_products_heading( $heading ) {
			return __( 'Related Products', 'blogshammir' );
		}
		...
		public function admin_redirects() {
			...
			if ( false !== strpos( $current_page, 'blogshammir' ) ) {
		...
		public function dynamic_css( $css ) {
			...
			$css .= '
				...
				.woocommerce div.product .woocommerce-tabs ul.tabs li a {
					color: ' . blogshammir_hex2rgba( $content_text_color, 0.73 ) . ';
				}
			...
			$css .= '
				...
				.woocommerce .quantity .blogshammir-woo-minus,
				.woocommerce .quantity .blogshammir-woo-plus {
			...
			$css .= '
				...
				.woocommerce ul.products li.product .blogshammir-loop-product__category-wrap a:hover {
					color: ' . blogshammir_option( 'content_link_hover_color' ) . ';
				}
			';
			...
			// Background.
			$header_background = blogshammir_option( 'header_background' );

			if ( 'color' === $header_background['background-type'] && $header_background['background-color'] ) {
				$css .= '
					.blogshammir-header-widget__cart .blogshammir-cart .blogshammir-cart-count { 
			...
			// Headings.
			$css .= blogshammir_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title, .woocommerce #reviews #comments h2, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'headings_font' );

			$css .= blogshammir_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title', 'h2_font' );
			$css .= blogshammir_dynamic_styles()->get_typography_field_css( '.woocommerce #reviews #comments h2', 'h3_font' );
			$css .= blogshammir_dynamic_styles()->get_typography_field_css( '.woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'h4_font' );
...
if ( ! function_exists( 'blogshammir_woocommerce' ) ) :
	/**
	 * The function which returns the one Blogshammir_Woocommerce instance.
	 *
	 * @since 1.0.0
	 * @return object
	 */
	function blogshammir_woocommerce() {
		return Blogshammir_Woocommerce::instance();
	}
endif;

blogshammir_woocommerce();
