 ...
 * @since 1.0.0
 */
var blogshammirGetIndex = function (el) {
	var i = 0;
 ...
 */
var blogshammirSlideUp = (target, duration = 500) => {
 ...
 * @param  {Number} duration Animation duration.
 */
var blogshammirSlideDown = (target, duration = 500) => {
 ...
 */
var blogshammirScrollTo = (function () {
	...
	/**
	 * blogshammirScrollTo Constructor
	 * @param {object} options Options
	 * @param {object} easeFunctions Custom ease functions
	 */
	function blogshammirScrollTo() {
	 ...
	 */
	blogshammirScrollTo.prototype.registerTrigger = function (dom, callback) {
		var _this = this;
	 ...
	 */
	blogshammirScrollTo.prototype.move = function (target) {
		var _this2 = this;
	 ...
	 */
	blogshammirScrollTo.prototype.addEaseFunction = function (name, fn) {
 ...
 */
var blogshammirGetParents = (elem, selector) => {
 ...
 */
var blogshammirTriggerEvent = function (el, typeArg) {
...
(function () {
	...
	var blogshammirScrollButton = document.querySelector('#blogshammir-scroll-top');
	...
	*/
	 ...
	 * @since 1.0.0
	 */
	var blogshammirSmartSubmenus = () => {
		if (document.body.classList.contains('blogshammir-is-mobile')) {
			return;
		}
	 ...
	 * @since 1.0.0
	 */
	var blogshammirScrollTopButton = () => {
		if (null === blogshammirScrollButton) {
			return;
		}

		if (450 < window.pageYOffset || 450 < document.documentElement.scrollTop) {
			blogshammirScrollButton.classList.add('blogshammir-visible');
		} else {
			blogshammirScrollButton.classList.remove('blogshammir-visible');
		}
	};
	 ...
	 * @since 1.0.0
	 */
	var blogshammirStickyHeader = () => {

		// Check if sticky is enabled.
		if (!blogshammir_vars['sticky-header'].enabled) {
			return;
		}

		var header = document.getElementById('blogshammir-header');
		var headerInner = document.getElementById('blogshammir-header-inner');
		var wpadminbar = document.getElementById('wpadminbar');

		// Check for header layout 3 & 4.
		if (document.body.classList.contains('blogshammir-header-layout-3') || document.body.classList.contains('blogshammir-header-layout-4') || document.body.classList.contains('blogshammir-header-layout-6')) {
			header = document.querySelector('#blogshammir-header .blogshammir-nav-container');
			headerInner = document.querySelector('#blogshammir-header .blogshammir-nav-container .blogshammir-container');
		}

		// Mobile nav active.
		if (window.outerWidth <= blogshammir_vars['responsive-breakpoint']) {
			var header = document.getElementById('blogshammir-header');
			var headerInner = document.getElementById('blogshammir-header-inner');
		}
		...
		var maybeStickHeader = function () {
			let hideOn = blogshammir_vars['sticky-header']['hide_on'];
			...
			if (sticky) {
				if (!document.body.classList.contains('blogshammir-sticky-header')) {
					stickyPlaceholder = document.createElement('div');
					stickyPlaceholder.setAttribute('id', 'blogshammir-sticky-placeholder');

					stickyPlaceholder.style.height = headerInner.getBoundingClientRect().height + 'px';
					header.appendChild(stickyPlaceholder);

					document.body.classList.add('blogshammir-sticky-header');

					// Add sticky header offset variable.
					document.body.style.setProperty('--blogshammir-sticky-h-offset', header.offsetHeight + 20 + 'px');
				}
			} else {
				if (document.body.classList.contains('blogshammir-sticky-header')) {
					document.body.classList.remove('blogshammir-sticky-header');
					document.getElementById('blogshammir-sticky-placeholder').remove();
				}

				// Remove sticky header offset variable.
				document.body.style.removeProperty('--blogshammir-sticky-h-offset');
			}
		};
		...
		if ('true' !== header.getAttribute('data-scroll-listener')) {
			window.addEventListener('scroll', function () {
				blogshammirDebounce(checkPosition());
			});
		...
		// Trigger scroll.
		blogshammirTriggerEvent(window, 'scroll');
	};

	/**
	 * Handles smooth scrolling of elements that have 'blogshammir-smooth-scroll' class.
	 *
	 * @since 1.0.0
	 */
	var blogshammirSmoothScroll = () => {
		const scrollTo = new blogshammirScrollTo({
			tolerance: null === document.getElementById('wpadminbar') ? 0 : document.getElementById('wpadminbar').getBoundingClientRect().height
		});

		const scrollTriggers = document.getElementsByClassName('blogshammir-smooth-scroll');
	 ...
	 * @since 1.0.0
	 */
	var blogshammirMenuAccessibility = () => {
		if (!document.body.classList.contains('blogshammir-menu-accessibility')) {
			return;
		}

		document.querySelectorAll('.blogshammir-nav').forEach((menu) => {
			...
			menu.querySelectorAll('a').forEach((link) => {
				link.addEventListener('focus', blogshammirMenuFocus, true);
				link.addEventListener('blur', blogshammirMenuFocus, true);
			});
		});
	};
	 ...
	 * @since 1.0.0
	 */
	function blogshammirMenuFocus() {
		var self = this;

		// Move up until we find .blogshammir-nav
		while (!self.classList.contains('blogshammir-nav')) {
	 ...
	 * @since 1.0.0
	 */
	var blogshammirCalcScreenWidth = () => {
		document.body.style.setProperty('--blogshammir-screen-width', document.body.clientWidth + 'px');
	};
	 ...
	 * @since 1.0.0
	 */
	var blogshammirDropdownDelay = () => {
		var hoverTimer = null;

		document.querySelectorAll('.blogshammir-nav .menu-item-has-children').forEach((item) => {
	 ...
	 * @since 1.0.0
	 */
	var blogshammirCartDropdownDelay = () => {
		var hoverTimer = null;

		document.querySelectorAll('.blogshammir-header-widget__cart .blogshammir-widget-wrapper').forEach((item) => {
			item.addEventListener('mouseenter', function () {
	 ...
	 * @since 1.0.0
	 */
	var blogshammirHeaderSearch = () => {
		var searchButton = document.querySelectorAll('.blogshammir-search');
		...
		searchButton.forEach((item) => {
			item.addEventListener('click', (e) => {
				...
				if (item.classList.contains('blogshammir-active')) {
					close_search(item);
				} else {
					show_search(item);
				}
		...
		document.querySelectorAll('.blogshammir-search-close').forEach(item => item.addEventListener('click', function (e) {
			e.preventDefault();
			if (!item.classList.contains('blogshammir-active')) {
				close_search(document.querySelector('.blogshammir-search'));
				document.querySelector('.blogshammir-search').focus();
			}
		}));
		...
		var show_search = function (item) {
			...
			var blogshammirsearch = document.querySelectorAll('.blogshammir-search-form');
			blogshammirsearch.forEach(function (blogshammirsearch) {
				if (!blogshammirsearch) {
					return false;
				}
				document.addEventListener('keydown', function (e) {
					var firstEl, selectors, elements, lastEl, tabKey, shiftKey;
					selectors = 'input, a, button';
					elements = blogshammirsearch.querySelectorAll(selectors);
			...
			// Make search visible
			document.body.classList.add('blogshammir-search-visible');

			setTimeout(function () {

				// Highlight the search icon
				item.classList.add('blogshammir-active');
		...
		var close_search = function (item) {
			...
			// Animate out
			document.body.classList.remove('blogshammir-search-visible');

			// Unhighlight the search icon
			item.classList.remove('blogshammir-active');
		...
		var esc_close_search = function (e) {
			if (27 == e.keyCode) {
				document.querySelectorAll('.blogshammir-search').forEach((item) => {
					close_search(item);
				});
			}
		};
		...
		var outside_close_search = function (e) {
			if (null === e.target.closest('.blogshammir-search-container') && null === e.target.closest('.blogshammir-search')) {
				document.querySelectorAll('.blogshammir-search').forEach((item) => {
					close_search(item);
				});
			}
	 ...
	 * @since 1.0.0
	 */
	var blogshammirMobileMenu = () => {
		var page = pageWrapper,
			nav = document.querySelector('#blogshammir-header-inner .blogshammir-nav'),
			current;

		document.querySelectorAll('.blogshammir-mobile-nav > button').forEach((item) => {
		...
		var show_menu = function (e) {
			...
			var blogshammirnav = document.querySelectorAll('.site-header');
			if (document.body.classList.contains('blogshammir-is-mobile')) {
				blogshammirnav.forEach(function (blogshammirnav) {
					if (!blogshammirnav) {
						return false;
					}
					document.addEventListener('keydown', function (e) {
						var firstEl, selectors, elements, lastEl, tabKey, shiftKey, closeIcon;
						selectors = '.blogshammir-nav a, .blogshammir-nav button';
						elements = blogshammirnav.querySelectorAll(selectors);
						lastEl = elements[elements.length - 1];
						firstEl = elements[0];
						tabKey = 9 === e.keyCode;
						shiftKey = e.shiftKey;
						closeIcon = document.querySelector('.blogshammir-hamburger-blogshammir-primary-nav');
						if (!shiftKey && tabKey && lastEl === document.activeElement) {
							e.preventDefault();
							closeIcon.focus();
						}
			...
			document.querySelectorAll('#blogshammir-header .blogshammir-nav').forEach((item) => {
				item.addEventListener('click', submenu_toggle);
			});

			// Slide down the menu.
			blogshammirSlideDown(nav, 350);
		};
		...
		var close_menu = function (e) {
			...
			// Close submenus
			document.querySelectorAll('#blogshammir-header .blogshammir-nav > ul > .blogshammir-open').forEach((item) => {
		...
		var outside_close_menu = function (e) {
			if (null === e.target.closest('.blogshammir-hamburger') && null === e.target.closest('.site-navigation')) {
				close_menu();
			}
		};
		...
		var submenu_display_toggle = (current) => {
			if (current.classList.contains('blogshammir-open')) {
				current.classList.remove('blogshammir-open');
				current.querySelectorAll('.sub-menu').forEach((submenu) => {

					//submenu.style.display = null;
					blogshammirSlideUp(submenu, 350);
				});

				// Close all submenus automatically.
				current.querySelectorAll('li').forEach((item) => {
					item.classList.remove('blogshammir-open');
				...
				current.querySelectorAll('.sub-menu').forEach((submenu) => {
					...
					if (current === submenu.parentElement) {
						...
						blogshammirSlideDown(submenu, 350);
					}
				});

				current.classList.add('blogshammir-open');
			}
		};

		// Create custom event for closing mobile menu.
		document.addEventListener('blogshammir-close-mobile-menu', close_menu);
	};

	/**
	 * Blogshammir preloader.
	 *
	 * @since 1.0.0
	 */
	var blogshammirPreloader = (timeout = 0) => {
		var preloader = document.getElementById('blogshammir-preloader');
		...
		var hide_preloader = () => {
			if (document.body.classList.contains('blogshammir-loaded')) {
				return;
			}

			// Start fade out animation.
			document.body.classList.add('blogshammir-loading');

			setTimeout(function () {

				// Fade out animation completed - set display none
				document.body.classList.replace('blogshammir-loading', 'blogshammir-loaded');

				// Dispatch event when preloader is done
				blogshammirTriggerEvent(document.body, 'blogshammir-preloader-done');
			}, delay);
		};
	 ...
	 * @since 1.0.0
	 */
	var blogshammirToggleComments = () => {
		if (!document.body.classList.contains('blogshammir-has-comments-toggle')) {
			return;
		}

		if (null == document.getElementById('blogshammir-comments-toggle')) {
			return;
		}
		...
		var toggleComments = (e) => {
			...
			if (document.body.classList.contains('comments-visible')) {
				...
				document.getElementById('blogshammir-comments-toggle').querySelector('span').innerHTML = blogshammir_vars.strings.comments_toggle_show;
			} else {
				document.body.classList.add('comments-visible');
				document.getElementById('blogshammir-comments-toggle').querySelector('span').innerHTML = blogshammir_vars.strings.comments_toggle_hide;
			}
		};

		if (null !== document.getElementById('blogshammir-comments-toggle') && (-1 !== location.href.indexOf('#comment') || -1 !== location.href.indexOf('respond'))) {
			toggleComments();
		}

		document.getElementById('blogshammir-comments-toggle').addEventListener('click', toggleComments);
	};
	...
	var blogshammirCommentsClick = () => {
		...
		commentsLink.addEventListener('click', function (e) {
			...
			if (document.body.classList.contains('blogshammir-has-comments-toggle') && !document.body.classList.contains('comments-visible')) {
				document.getElementById('blogshammir-comments-toggle').click();
			}
		});
	};
	 ...
	 * @since 1.0.0
	 */
	var blogshammirCheckMobileMenu = () => {

		// Update body class if mobile breakpoint is reached.
		if (window.innerWidth <= blogshammir_vars['responsive-breakpoint']) {
			document.body.classList.add('blogshammir-is-mobile');
		} else {
			if (document.body.classList.contains('blogshammir-is-mobile')) {
				document.body.classList.remove('blogshammir-is-mobile');
				blogshammirTriggerEvent(document, 'blogshammir-close-mobile-menu');
			}
		}
	};
	 ...
	 * @since 1.0.0
	 */
	var blogshammirSliderInit = (sliderElement) => {
	 ...
	 * @since 1.0.0
	 */
	var blogshammirUpdateTimeAndDate = () => {
		if (document.getElementById("blogshammir-time")) {
			document.getElementById("blogshammir-time").textContent = new Date().toLocaleTimeString();
		}
		if (document.getElementById("blogshammir-date")) {
			const options = { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' };
			document.getElementById("blogshammir-date").textContent = new Date().toLocaleString(document.documentElement.lang, options);
		}
	}
	 ...
	 * @since 1.0.0
	 */
	var blogshammirHandleQuickLike = (element) => {
	 ...
	 * @since 1.0.0
	 */
	var blogshammirTicker = () => {
		const hdir = document.body.classList.contains("rtl") ? "right" : "left";
		const htc = document.querySelector(".blogshammir-ticker.one-ticker");
		if (htc) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		blogshammirPreloader(5000);
		blogshammirMenuAccessibility();
		blogshammirKeyboardFocus();
		blogshammirScrollTopButton();
		blogshammirSmoothScroll();
		blogshammirDropdownDelay();
		blogshammirToggleComments();
		blogshammirHeaderSearch();
		blogshammirMobileMenu();
		blogshammirCheckMobileMenu();
		blogshammirSmartSubmenus();
		blogshammirCommentsClick();
		blogshammirCartDropdownDelay();
		blogshammirStickyHeader();
		blogshammirCalcScreenWidth();

		// Initialize all sliders, including those that are not thumbs
		document.querySelectorAll('.blogshammir-swiper:not(.swiper-bottom)').forEach(blogshammirSliderInit);
		// Then, initialize thumb (bottom) sliders to ensure they can be linked
		document.querySelectorAll('.blogshammir-swiper.swiper-bottom').forEach(blogshammirSliderInit);

		setInterval(blogshammirUpdateTimeAndDate, 1000);
		blogshammirUpdateTimeAndDate(); // Initial call to display immediately
	...
	window.addEventListener('load', function () {
		blogshammirPreloader();
	});

	// Scroll
	window.addEventListener('scroll', function () {
		blogshammirDebounce(blogshammirScrollTopButton());
	});

	// Resize
	window.addEventListener('resize', function () {
		blogshammirDebounce(blogshammirSmartSubmenus());
		blogshammirDebounce(blogshammirCheckMobileMenu());
		blogshammirDebounce(blogshammirCalcScreenWidth());
	});

	// Blogshammir ready
	blogshammirTriggerEvent(document.body, 'blogshammir-ready');

	//--------------------------------------------------------------------//
	// Global
	//--------------------------------------------------------------------//

	window.blogshammir = window.blogshammir || {};

	// Make these function global.
	window.blogshammir.preloader = blogshammirPreloader;
	window.blogshammir.stickyHeader = blogshammirStickyHeader;
	window.blogshammirSliderInit = blogshammirSliderInit;
	...
	const Cursor = (() => {
		const cursor = document.querySelector('.blogshammir-js-cursor');
		...
		const setVariables = () => {
			if (!cursor) return;
			[follower, label, icon] = ['.blogshammir-js-follower', '.blogshammir-js-label', '.blogshammir-js-icon'].map(sel => cursor.querySelector(sel));
	...
	document.addEventListener('DOMContentLoaded', function () {
		const toggleButton = document.querySelector('.blogshammir-darkmode');
		if (toggleButton) {
			// Initialize dark mode based on customizer setting or local storage
			let darkModeEnabled = blogshammir_vars.dark_mode || localStorage.getItem('darkmode') === 'dark';
